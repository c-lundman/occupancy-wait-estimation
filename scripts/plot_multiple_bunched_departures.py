#!/usr/bin/env python3
"""Plot inflow/outflow/queue diagnostics for bunched-departures scenario."""
...
def _bin_counts(df: pd.DataFrame, col: str = "timestamp") -> pd.Series:
    ts = pd.to_datetime(df[col], utc=True, format="mixed")
    return ts.dt.floor("1min").value_counts().sort_index()


def main() -> None:
    root = Path("data/scenarios/multiple_bunched_departures")
    out_png = Path("local/plots/multiple_bunched_departures_wout100_eps001_mult2_diagnostics.png")
