#!/usr/bin/env python3
"""Benchmark max-queue reconstruction error for bunched-departures scenario."""
...
def _true_queue_from_perfect(in_df: pd.DataFrame, out_df: pd.DataFrame) -> pd.Series:
    ts_in = pd.to_datetime(in_df["timestamp"], utc=True, format="mixed")
    ts_out = pd.to_datetime(out_df["timestamp"], utc=True, format="mixed")
